...
<style>
  * { margin:0; padding:0; box-sizing:border-box; }
  ...
  canvas { display:block; cursor:pointer; image-rendering:pixelated; touch-action:none; -webkit-tap-highlight-color:transparent; }
  #controls {
    width:100%; max-width:900px; padding:8px 16px;
  ...
  #rotateHint {
    display:none; position:fixed; top:0; left:0; width:100%; height:100%;
    background:#0d1117; z-index:9999; flex-direction:column;
    justify-content:center; align-items:center; text-align:center; padding:20px;
  }
  #rotateHint.show { display:flex; }
  #rotateHint .rotate-icon {
    font-size:64px; margin-bottom:20px; animation:rotateAnim 1.5s ease-in-out infinite;
  }
  @keyframes rotateAnim {
    0%,100% { transform:rotate(0deg); }
    50% { transform:rotate(90deg); }
  }
  #rotateHint .rotate-title { font-size:20px; color:#e94560; margin-bottom:8px; }
  #rotateHint .rotate-sub { font-size:14px; color:#888; }

  /* ===== 手机响应式布局（横屏）===== */
  @media (max-width: 920px) and (hover: none) and (pointer: coarse) {
    body { font-size:13px; }
    #header {
      max-width:100%; padding:6px 10px; gap:6px; flex-wrap:nowrap; overflow:hidden;
    }
    #header h1 { font-size:14px; white-space:nowrap; }
    #levelSelector { gap:3px; overflow-x:auto; max-width:45vw; scrollbar-width:none; }
    #levelSelector::-webkit-scrollbar { display:none; }
    .level-btn { width:26px; height:26px; font-size:11px; flex-shrink:0; }
    #mpIndicator { font-size:10px; padding:2px 6px; }
    #infoBar {
      max-width:100%; padding:4px 10px; gap:8px; font-size:11px;
      overflow-x:auto; scrollbar-width:none; flex-wrap:nowrap;
    }
    #infoBar::-webkit-scrollbar { display:none; }
    #infoBar .level-desc { min-width:auto; font-size:11px; white-space:nowrap; }
    #canvasWrap {
      margin:4px 0; border-width:2px; max-width:100vw; overflow:hidden;
    }
    canvas { max-width:100%; height:auto; max-height:calc(100vh - 220px); }
    #controls {
      max-width:100%; padding:4px 10px; gap:4px;
    }
    .btn { padding:8px 10px; font-size:12px; }
    .btn.primary { padding:8px 14px; }
    #controls .desktop-only { display:none !important; }
    #controls .mobile-only { display:inline !important; }
    #componentBar {
      max-width:100%; padding:4px 10px; gap:4px; overflow-x:auto; scrollbar-width:none;
    }
    #componentBar::-webkit-scrollbar { display:none; }
    #componentBar .label { font-size:10px; white-space:nowrap; }
    .comp-btn { width:44px; height:44px; flex-shrink:0; }
    .comp-btn .name { font-size:7px; }
    #hint { max-width:100%; padding:3px 10px; font-size:10px; }
    /* 聊天面板改为底部弹出 */
    #chatPanel {
      left:0 !important; top:auto !important; bottom:0;
      width:100% !important; height:200px !important; max-height:50vh;
      border-right:none; border-top:2px solid #0f3460;
    }
    body.chat-open { padding-left:0 !important; padding-bottom:200px; }
    /* 弹窗缩小 */
    #winModal .panel, #failModal .panel, #finalModal .panel { padding:24px 20px; max-width:90vw; max-height:85vh; overflow-y:auto; }
    #winModal h2 { font-size:28px; }
    #winModal p { font-size:14px; }
    #finalModal h2 { font-size:36px; }
    #finalModal p { font-size:16px; }
    #introModal .panel { padding:14px 16px; max-width:92vw; max-height:80vh; overflow-y:auto; }
    #introModal h2 { font-size:16px; margin-bottom:4px; }
    #introModal .subtitle { font-size:10px; margin-bottom:8px; }
    #introModal .intro-desc { font-size:12px; line-height:1.6; margin-bottom:10px; }
    #introModal .intro-tip { font-size:11px; margin-bottom:10px; }
    #introModal .btn { font-size:14px; padding:10px 24px; }
    #onlineModal .panel { padding:16px 20px; max-width:90vw; max-height:85vh; overflow-y:auto; }
    /* 主页适配 */
    #homePage .home-title { font-size:32px; }
    #homePage .home-subtitle { font-size:13px; }
    #homePage .home-level-btn { width:40px; height:40px; font-size:16px; }
    #homePage .home-start { padding:12px 36px; font-size:16px; }
    #homePage .home-free { padding:10px 28px; font-size:14px; }
  }
</style>
</head>
<body>
...
<div id="rotateHint">
  ...
  <div class="rotate-title">请横放手机</div>
  <div class="rotate-sub">游戏在横屏模式下体验最佳</div>
</div>
...
<div id="controls">
  ...
  <span style="font-size:12px;color:#888" class="desktop-only">左键放置 | 右键移除 | R键旋转</span>
  <span style="font-size:11px;color:#888;display:none" class="mobile-only">点击放置 | 长按删除</span>
</div>
...
canvas.addEventListener('touchstart', (e) => {
  ...
  touchStartPos = {
    ...
    gx: Math.floor((touch.clientX - rect.left) * (canvas.width / rect.width) / CELL),
    gy: Math.floor((touch.clientY - rect.top) * (canvas.height / rect.height) / CELL),
  };
  touchLongPressed = false;
  touchTimer = setTimeout(() => {
    touchLongPressed = true;
    if (!touchStartPos) return;
    let gx = touchStartPos.gx, gy = touchStartPos.gy;
    if (gx < 0 || gx >= G.cols || gy < 0 || gy >= G.rows) return;
    if (MP.connected && !isInMyZone(gx)) { showTip('这是队友的区域，你不能在此操作'); return; }
    let cell = G.grid[gx][gy];
    if (cell.type === T.EMPTY || cell.type === T.INPUT || cell.type === T.OUTPUT) return;
...
canvas.addEventListener('touchend', (e) => {
  if (touchTimer) { clearTimeout(touchTimer); touchTimer = null; }
  // 如果长按已触发，阻止后续的click事件
  if (touchLongPressed) { e.preventDefault(); touchLongPressed = false; }
}, { passive: false });
